...
If we have a Collection of a lot of many items and we need to find items very often.
- e.g. because we need to remove them again
- or because we want to check, whether a certain item is already insider
...
Let's look at a collision. We want to insert Martha into the Hash-Table:
...
Now, let's look at a collision again. We want to insert Martha into the Hash-Table:
...
- `bool Insert(T item)` returns false, if it was already inside
- `bool Exists(T item)` returns true, if it is inside
- `bool Remove(T item)` returns false, if it was inside
